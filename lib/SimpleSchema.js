...
function checkSchemaOverlap(schema) {
  _.each(schema, (val, key) => {
      // triggers errors in case you forget to set the type of sub-items
      if (!val.type) throw new Error(`The type for "${key}" is set to a SimpleSchema instance that defines "${key}.${subKey}", but the parent SimpleSchema instance also tries to define "${key} type"`);
