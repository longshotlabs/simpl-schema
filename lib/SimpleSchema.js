...
class SimpleSchema {
  ...
  extend(schema = {}) {
    ...
    // Mutate the clone to add all the subschema props to the main schema
    expandSchema(this._schema);
    ...
    _.each(this._schema, (definition, fieldName) => {
      this._schema[fieldName] = definition = checkAndScrubDefinition(fieldName, definition, this._constructorOptions);
