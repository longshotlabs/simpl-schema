...
describe('SimpleSchema', function () {
  describe('nesting', function () {
    it('works', function () {
      const childDef = {
        type: SimpleSchema.oneOf({ type: String, min: 10 }),
        label: 'foo',
        optional: false,
      };

      const parentDef = {
        type: SimpleSchema.oneOf({ type: SimpleSchema.Integer, min: 10 }),
        label: 'foo',
        optional: false,
      };
      ...
      const defs = parent.schema();

      // should change parent definition types to Object
      expect(defs.value.type).toEqual(SimpleSchema.oneOf(Object));
      ...
      expect(defs.array.type).toEqual(SimpleSchema.oneOf(Array));

      // should add array child definitions to parent schema
      expect(defs['array.$'].type).toEqual(SimpleSchema.oneOf(Object));
